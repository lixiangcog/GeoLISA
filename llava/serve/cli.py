...
def main(args):
    ...
    while True:
        ...
        # 设置 pad_token_id，假设模型有 eos_token_id
        pad_token_id = model.config.eos_token_id

        # 移除停止条件，确保模型生成更长的文本
        stopping_criteria = []  # 移除停止条件
        temperature = 0.7  # 增加温度，增加生成的多样性
        max_new_tokens = 1024  # 增加最大生成令牌数，确保生成的文本更长
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                input_ids,
                attention_mask=attention_mask,  # 传递 attention mask
                images=image_tensor,
                pad_token_id=pad_token_id,  # 传递 pad_token_id
                do_sample=True if temperature > 0 else False,
                temperature=temperature,
                max_new_tokens=max_new_tokens,  # 增加最大生成令牌数
                streamer=streamer,
                use_cache=True,
                stopping_criteria=stopping_criteria  # 使用空的停止条件
            )
