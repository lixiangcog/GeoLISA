...
class GPT(LLM):
  ...
  async def dispatch_openai_requests(
    ...
    # Asynchronously call the function for each anchor
  ...
  def split_input(self, fixed_instruction, few_shot_examples, splittable_input, input_header, output_header):
    ...
    remaining_token_len = math.ceil((self.anchor_percent*self.max_length)-len(fixed_token_ids))
    
    # Tokenize splittable_input
    split_token_ids = self.encoding.encode(splittable_input)

    # Split tokenized split_anchor into list of individual inputs strings. Uses tokens to calculate length
    ...
    # Take the fixed_anchor, few_shot_examples, splitted inputs, and input/output headers and generate list of anchor strings.
