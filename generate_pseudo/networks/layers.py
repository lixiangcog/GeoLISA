...
def linear(
    inputs, weight, bias, meta_step_size=0.001, meta_loss=None, stop_gradient=False
):
    inputs = inputs.cuda()
    weight = weight.cuda()
    bias = bias.cuda()
    ...
    if meta_loss is not None:
        ...
        if not stop_gradient:
            ...
            grad_weight = Variable(
                autograd.grad(meta_loss, weight, create_graph=True)[0].data,
                requires_grad=False,
            )

            if bias is not None:
                grad_bias = Variable(
                    autograd.grad(meta_loss, bias, create_graph=True)[0].data,
                    requires_grad=False,
                )
                bias_adapt = bias - grad_bias * meta_step_size
            else:
