...
def draw_boundary(prediction, save_root, name):
    ...
    cv2.imwrite(
        os.path.join(save_root, "boundary", name.split(".")[0]) + ".png", boundary
    )
...
def postprocessing(prediction, threshold=0.75, dataset="G"):
    if dataset[0] == "D":
        ...
        disc_mask = morphology.binary_erosion(disc_mask, morphology.diamond(7)).astype(
            np.uint8
        )  # return 0,1
        cup_mask = morphology.binary_erosion(cup_mask, morphology.diamond(7)).astype(
            np.uint8
        )  # return 0,1
...
def save_per_img(
    patch_image, data_save_path, img_name, prob_map, mask_path=None, ext="bmp"
):
    path1 = os.path.join(data_save_path, "overlay", img_name.split(".")[0] + ".png")
    path0 = os.path.join(
        data_save_path, "original_image", img_name.split(".")[0] + ".png"
    )
    ...
    disc_mask = morphology.binary_dilation(disc_mask, morphology.diamond(7)).astype(
        np.uint8
    )  # return 0,1
    cup_mask = morphology.binary_dilation(cup_mask, morphology.diamond(7)).astype(
        np.uint8
    )  # return 0,1
    ...
    for n, contour in enumerate(contours_cup):
        patch_image[(contour[:, 0]).astype(int), (contour[:, 1]).astype(int), :] = [
            0,
            255,
            0,
        ]
        patch_image[
            (contour[:, 0] + 1.0).astype(int), (contour[:, 1]).astype(int), :
        ] = [0, 255, 0]
        patch_image[
            (contour[:, 0] + 1.0).astype(int), (contour[:, 1] + 1.0).astype(int), :
        ] = [0, 255, 0]
        patch_image[
            (contour[:, 0]).astype(int), (contour[:, 1] + 1.0).astype(int), :
        ] = [0, 255, 0]
        patch_image[
            (contour[:, 0] - 1.0).astype(int), (contour[:, 1]).astype(int), :
        ] = [0, 255, 0]
