...
class add_salt_pepper_noise:
    def __call__(self, sample):
        ...
        if seed > 0.75:
            # Add Salt noise
            coords = [
                np.random.randint(0, i - 1, int(num_salt)) for i in X_imgs_copy.shape
            ]
            X_imgs_copy[coords[0], coords[1], :] = 1
        elif seed > 0.5:
            # Add Pepper noise
            coords = [
                np.random.randint(0, i - 1, int(num_pepper)) for i in X_imgs_copy.shape
            ]
        ...
        if seed > 0.5:
            ...
            for i in range(3):
                ...
                transformed_image[:, :, i] = map_coordinates(
                    np.array(image)[:, :, i], indices, order=1
                ).reshape(shape)
                # break
            if label is not None:
                transformed_label[:, :] = map_coordinates(
                    np.array(label)[:, :], indices, order=1, mode="nearest"
                ).reshape(shape)
            else:
...
class GetBoundary(object):
    ...
    def __call__(self, mask):
        ...
        dila_disc = ndimage.binary_dilation(disc, iterations=self.width).astype(
            disc.dtype
        )
        eros_disc = ndimage.binary_erosion(disc, iterations=self.width).astype(
            disc.dtype
        )
...
class Normalize_tf(object):
    ...
    def __call__(self, sample):
        ...
        return {"image": img, "map": mask, "boundary": boundary, "img_name": name}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {"image": img, "map": map, "boundary": boundary, "img_name": name}
