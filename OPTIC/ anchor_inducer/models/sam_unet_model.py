...
class SAM_UNET(nn.Module):
    ...
    def __init__(
        ...
        self.anchor_encoder = SAManchorEncodeWrapper(ori_sam=ori_sam, fix=False)
    ...
    def infer(self, list_input: List[Dict[str, Any]]) -> List[Dict[str, torch.Tensor]]:
        ...
        If anchors are not known in advance, using SamPredictor [TODO] is
        ...
        Arguments:
          ...
          global_anchor (bool): Whether to use a global anchor for all images.
        ...
        Arguments:
          list_input (list(dict)): A list over input images, each a
            ...
            excluded if it is not present.
              ...
              'original_size': (tuple(int, int)) The original size of
                the image before transformation, as (H, W).
              'point_coords': (torch.Tensor) Point anchors for
                this image, with shape num_masksxNxnum_pointsx2. Already transformed to the
                input frame of the model.
              'point_labels': (torch.Tensor) Labels for point anchors,
