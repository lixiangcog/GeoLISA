...
from .sam_unet_model import SAM_UNET
...
def _build_sam(type, ori_sam, ori_checkpoint: bool, sam_unet_checkpoint: str):
    if sam_unet_checkpoint is not None and ori_checkpoint == False:
        ...
        model = SAM_UNET(
            resnet_type=type, ori_sam=ori_sam(None), is_resnet_pretrained_or_not=False
        )
        model.load_state_dict(sam_unet_checkpoint)
    elif ori_checkpoint == True:
        model = SAM_UNET(
            resnet_type=type,
            ori_sam=ori_sam(config_dict["checkpoint_path"]),
            is_resnet_pretrained_or_not=True,
        )
    else:
        model = SAM_UNET(
            resnet_type=type, ori_sam=ori_sam(None), is_resnet_pretrained_or_not=False
