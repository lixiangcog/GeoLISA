...
class SAMImageEncodeWrapper(nn.Module):
    def __init__(self, ori_sam, fix: bool = True):
        ...
        self.anchor_dim = self.sam_img_encoder.embed_dim
    ...
    def forward(self, x, anchor_tokens: torch.Tensor = None):

        # anchor_tokens [b, depth, num_anchors, anchor_dim]
        # print(x.shape)
        ...
        for idx, blk in enumerate(self.sam_img_encoder.blocks):
            current_anchor = (
                anchor_tokens[:, idx, :, :] if anchor_tokens is not None else None
            )
            # current_anchor [b, num_anchors, anchor_dim]
            x = blk(x, anchor_tokens=current_anchor)
...
class SAManchorEncodeWrapper(nn.Module):
    def __init__(self, ori_sam, fix: bool = True):
        super(SAManchorEncodeWrapper, self).__init__()
        self.sam_anchor_encoder = ori_sam.anchor_encoder
        if fix:
            for name, param in self.sam_anchor_encoder.named_parameters():
                param.requires_grad = False

    def forward(self, points=None, boxes=None, masks=None):
        sparse_embeddings, dense_embeddings = self.sam_anchor_encoder(
            points, boxes, masks
        )
        return sparse_embeddings, dense_embeddings

    def get_dense_pe(self):
        return self.sam_anchor_encoder.get_dense_pe()
...
class SAMMaskDecoderWrapper_Med(nn.Module):
    ...
    def forward(
        ...
        sparse_anchor_embeddings: torch.Tensor,
        dense_anchor_embeddings: torch.Tensor,
        ...
        masks = self.predict_masks(
            ...
            image_pe=image_pe,
            sparse_anchor_embeddings=sparse_anchor_embeddings,
            dense_anchor_embeddings=dense_anchor_embeddings,
    ...
    def predict_masks(
        ...
        output_tokens = output_tokens.unsqueeze(0).expand(
            sparse_anchor_embeddings.size(0), -1, -1
        )
        tokens = torch.cat((output_tokens, sparse_anchor_embeddings), dim=1)

        # Expand per-image data in batch direction to be per-mask
        src = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)
        src = src + dense_anchor_embeddings
