...
class GradCAM(object):
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        if isinstance(logit, tuple):
            logit = logit[0]  # segmentation 输出，通常是 pred_logit

        if logit.ndim == 4:
            # segmentation: [B, C, H, W]
            if class_idx is None:
                class_idx = 1  # 默认 cup 类
            score = logit[:, class_idx, :, :].mean()  # 平均池化，避免显存爆炸
        else:
            # classification: [B, C]
            if class_idx is None:
...
class GradCAMpp(GradCAM):
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        saliency_map = F.relu(saliency_map)
        saliency_map = F.upsample(
            saliency_map, size=(512, 512), mode="bilinear", align_corners=False
        )
        saliency_map_min, saliency_map_max = saliency_map.min(), saliency_map.max()
        saliency_map = (
            (saliency_map - saliency_map_min)
